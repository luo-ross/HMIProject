...
async function initDefaultKeys() {
    ...
    getRSASignKeyResult = await getRSAKeyAsync("Sign");
    //如果缓存里没有，则从新创建
    if (!getRSASignKeyResult.isSuccess) {
        //创建签名密钥
        getRSASignKeyResult = await generateRSAKeysAsync("Sign", "RSASSA-PKCS1-v1_5", ["sign", "verify"]);
        if (!getRSASignKeyResult.isSuccess) {
    ...
    getRSAEncryptKeyResult = await getRSAKeyAsync("Encrypt");
    if (!getRSAEncryptKeyResult.isSuccess) {
        //创建加密密钥
        getRSAEncryptKeyResult = await generateRSAKeysAsync("Encrypt", "RSA-OAEP", ["encrypt", "decrypt"]);
        if (!getRSAEncryptKeyResult.isSuccess) {
    ...
    //获取会话的Hash数据
    var getRSAHashResult = await getRSAHashAsync(arrayList);
    if (!getRSAHashResult.isSuccess) {
        ShowErrorMsg(getRSAHashResult.message);
        return;
    }

    var rsaSignDataResult = await rsaSignDataAsync(getRSAHashResult.data, getRSASignKeyResult.privateKey);
    ...
    const result = await makeAjaxRequestAsync("/api/v1/General/GetSessionModel", sessionRequestModel);
    var getSessionModelResult = await getSessionModel(result);
...
async function getSessionModel(operateResult)
    ...
    //验证签名
    var rsaVerifyDataResult = await rsaVerifyDataAsync(
    ...
    var aesKeyDecryptResult = await rsaDecryptAsync(aesKey, getRSAEncryptKeyResult.privateKey)
    ...
    var appIdDecryptResult = await rsaDecryptAsync(appId, getRSAEncryptKeyResult.privateKey)
...
async function generateRSAKeysAsync(
    keyName: string,
    name: string,
    keykeyUsages: ReadonlyArray<KeyUsage>):
    Promise<{ isSuccess: boolean, publicKey: string | null, privateKey: string | null, message: string }> {
    try {
...
async function getRSAHashAsync(arrayList: any[]):
...
async function rsaSignDataAsync(
    hash: ArrayBuffer,
    rsaSigningPrivateKey: string):
    Promise<{ isSuccess: boolean, data: any, message: string }> {
    try {
        // 检查私钥格式
...
//这里验证签名是用服务端的签名公钥
async function rsaVerifyDataAsync(
    hash: Uint8Array,
    signature: string,
    rsaSigningPrivateKey: string
): Promise<{ isSuccess: boolean, data: any, message: string }> {
...
async function rsaEncryptAsync(
...
async function rsaDecryptAsync(
    encryptContent: string,
    rsaEncryptionPrivateKey: string
): Promise<{ isSuccess: boolean, data: string | null, message: string }> {
    try {
        // 检查参数
        if (!encryptContent || typeof encryptContent !== 'string') {
