...
namespace RS.HMI.Client.Views.Logoin
{
    ...
    public partial class LoginView : RSWindow
    {
        ...
        {
            RSWinInfoBar rSWinInfo = new RSWinInfoBar();
            rSWinInfo.Show();
            ...
            Task.Factory.StartNew(async() =>
            {
                for (int i = 0; i < 10; i++)
                {
                    MessageBoxImage infoType = MessageBoxImage.None;
                    if (i % 2 == 0)
                    {
                        infoType = MessageBoxImage.Information;
                    }
                    else
                    {
                        infoType = MessageBoxImage.Exclamation;
                    }
                    this.RSWinInfoBar.AddMessageAsync($"这是消息{i + 1}", infoType);
            ...
            var operateResult = await this.LoginForm.InvokeLoadingActionAsync(async () =>
              {
                  ...
                  for (int i = 0; i < 10; i++)
                  {
                      ...
                      if (i % 2 == 0)
                      {
                          message.InfoType = MessageBoxImage.Information;
                      }
                      else
                      {
                          message.InfoType = MessageBoxImage.Exclamation;
                      }
                      rSWinInfo.InfoBarModelDataSource.Add(message);
